"""
main.py - オーケストレーター
フロー: fetch → score → generate(text+haiku) → validate → generate_image →
       post(parent with image) → post_reply(source URL) → log

PV関連性判定により無関係な記事はスキップして次の候補に進む。
会員専用記事(指定ドメイン×短文)もスキップ対象。
画像生成失敗(セーフティブロック等)時も次の候補にスキップ
(画像なし投稿の救済はなし)。
"""
import json
import sys
...
def run(dry_run: bool = False) -> int:
    ...
    for i, entry in enumerate(scored[:MAX_RETRIES]):
        ...
        # 画像生成 (失敗時は次の候補へ)
        haiku = result["generated"].get("haiku")
        print(f"  Haiku: {' / '.join(haiku) if haiku else '(missing)'}")
        print("  [image] Generating from haiku...")
        image_path = generate_image(haiku)
        if not image_path:
            print("  ⊘ Skipped (image generation failed)")
            skip_image_count += 1
            continue

        successful_entry = entry
        successful_post = result
        successful_image_path = image_path
        break

    if not successful_post:
        print(f"\n⚠ No valid post could be generated. "
              f"Skipped(PV無関係)={skip_count}件, "
              f"Skipped(会員専用)={skip_member_only_count}件, "
              f"Skipped(画像生成失敗)={skip_image_count}件. Exiting.")
        return 1

    post_text = successful_post["post"]
    haiku = successful_post["generated"]["haiku"]
    ...
    # 4. 親ポストを投稿
    try:
        result = post_to_x(post_text, image_path=successful_image_path)
    ...
    log_post({
        ...
        "skipped_image_failed": skip_image_count,
